...
module.exports = class ProjectView extends SymbolsView {
  ...
  async populate ({ retain = false } = {}) {
    ...
    let result = this.generateSymbols(editor, query, (symbols) => {
      ...
      console.debug('???? First symbols populated after', duration, 'ms:', symbols);
    ...
    // TODO: In a project-wide symbol search, we should assume that the list of
    // symbols we get will be dynamic and will need to be refreshed with each
    // character the user types in the search field. The provider should have a
    // way to indicate whether the returned list is static or dynamic, but I'm
